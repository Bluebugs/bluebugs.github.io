...
If the access is **not contiguous**, the lanes point to scattered locations in memory and the compiler must fall back to gather/scatter. On hardware that supports it, that is still one instruction but at 2-4x the latency. On WASM, which has no gather instruction, it means four separate scalar loads and an `insertelement` per lane. On AVX2 with byte-width data, it means 32 separate loads.

The performance difference is 4-8x. In our benchmarks, every example that achieves 5x or better has contiguous access in its hot loop. Every example that underperforms has a non-contiguous access somewhere the compiler could not resolve.

The contiguous recognizer matters more than loop peeling, mask elimination, or pattern detection. Those optimizations make a fast loop faster. The contiguous recognizer determines whether the loop is fast at all.

## Why Recognizing Contiguous Access Is Non-Trivial

If you have never looked at Go's SSA form, you might expect the index expression inside a range loop to be just the iteration variable, maybe plus a constant. In practice, it's not.
...
The logic is: first check if the index *is* the iter phi directly. If not, check if it's an ADD where one side is the iter phi and the other side is scalar. Try both sides of the ADD because the SSA does not guarantee operand order.
...
## Principles for other SIMD compilers

If you are building a vectorizing compiler for any managed-memory language:
